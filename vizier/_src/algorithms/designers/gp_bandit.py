...
Array = Union[types.Array, padding.PaddedArray]
...
class VizierGPBandit(vza.Designer, vza.Predictor):
  ...
  _features: Array = attr.field(init=False)
  # Numpy array representing the current recent trials' metrics.
  _labels: Array = attr.field(init=False)
  ...
  def _convert_trials_to_arrays(
    ...
    if self._padding_schedule:
      ...
      # Unpad labels. This is because the warper may take in to account all
      # labels.
    ...
    if self._padding_schedule:
      ...
      labels = padding.PaddedArray(padded_labels, pre_labels.is_missing)
