...
from vizier.raytune import SearchSpaceConverter
from vizier.service import clients
from vizier.service import pyvizier as vz


class VizierSearch(suggest.Searcher):
  """An OSS Vizier Searcher for Ray."""

  def __init__(
      self,
      study_id: Optional[str] = None,
      problem: Optional[vz.StudyConfig] = None,
      **kwargs,
  ):
  ...
  def set_search_properties(
    ...
    search_space = SearchSpaceConverter.to_vizier(config)
    vizier_goal = (
    ...
    study_config = vz.StudyConfig(
        ...
        metric_information=[
            vz.MetricInformation(self._metric, goal=vizier_goal)
        ],
    )
    self.study_client = clients.Study.from_study_config(
  ...
  def on_trial_result(self, trial_id, result):
    ...
    trial_client.add_measurement(
        vz.Measurement(result, elapsed_secs=elapsed_secs.total_seconds())
    )
  ...
  def on_trial_complete(self, trial_id, result=None, error=False):
    ...
    if error:
      ...
      trial_client.complete(infeasible_reason=f'Trial {trial_id} failed')
    else:
      measurement = None
      if result:
        elapsed_secs = (
  ...
  def suggest(self, trial_id):
    ...
    if not suggestions:
      return suggest.Searcher.FINISHED
