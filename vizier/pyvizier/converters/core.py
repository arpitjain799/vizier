...
class TrialToArrayConverter:
  ...
  def to_features(self, trials) -> np.ndarray:
    ...
    # Pad up the features.
    features = dict_to_array(self._impl.to_features(trials))
    return features

  def to_labels(self, trials) -> np.ndarray:
    """Returns the labels array with dimenion: (n_trials, n_metrics)."""
    # Pad up the labels.
    labels = dict_to_array(self._impl.to_labels(trials))
    return labels
  ...
  def to_parameters(self, arr: np.ndarray) -> Sequence[pyvizier.ParameterDict]:
    ...
    # Use the features_shape and labels_shape
...
class PaddedTrialToArrayConverter:
  """EXPERIMENTAL: Converts trials to arrays and pads / masks them."""

  _experimental_override = 'I am aware that this code may break at any point.'

  def __init__(
      self,
      impl: TrialToArrayConverter,
      experimental_override: str = '',
      *,
  ...
  def from_study_config(
    ...
    Returns:
      PaddedTrialToArrayConverter
    """
    converter = TrialToArrayConverter.from_study_config(
        study_config,
        scale=scale,
        pad_oovs=pad_oovs,
        max_discrete_indices=max_discrete_indices,
